...
object ArticleInterestScorer {
    fun score(interest: ArticleInterest?, title: String, keywords: List<String>): Int {
        if (interest == null) return keywordScore(title, keywords)
        ...
        return (explicit + behavior + keywordScore(title, keywords)).coerceIn(-100, 100)
    }
